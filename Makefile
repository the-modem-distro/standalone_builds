...
#
# Notes:
#	- Run 'make MACHINE=mdmXXXX [option]' to select the platform (mdm9607 is the default)
#	- Run 'make MACHINE=xxxx KERNEL_TREE=kernel_tree path to select a different kernel (default is themdm's current 3.18.140 kernel)
#
#
#
...
kernel_build:
	...
	$(CURRENT_PATH)/tools/dtbTool $(KERNEL_PATH)/build/arch/arm/boot/dts/qcom/ -s 4096 \
	-o $(KERNEL_OUTPUT_DTB) -p $(KERNEL_PATH)/build/scripts/dtc/ -v \
...
kernel_clean:
	rm -rf $(CURRENT_PATH)/kernel/build
